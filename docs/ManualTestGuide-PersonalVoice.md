...
7. **コンソールログを確認（重要）**
   ...
   [SpeechSynthesis] 音声名: DragonLatestNeural
...
### 成功基準
- ✅ Personal Voice ID が設定されている場合、常に `DragonLatestNeural` がベース音声として使用される
- ✅ 翻訳先言語を変更しても、同じ話者の声で合成される
- ✅ コンソールログに "Personal Voice モード" と表示される
- ✅ SSML に `DragonLatestNeural` が含まれる
...
### 成功基準
- ✅ すべての言語で同じベース音声 (`DragonLatestNeural`) が使用される
- ✅ すべての言語で同じ話者の声で合成される
...
### 成功基準
- ✅ `<voice name="DragonLatestNeural">` が使用されている
...
1. コンソールログを確認
   ...
   - ベース音声が `DragonLatestNeural` になっているか？
